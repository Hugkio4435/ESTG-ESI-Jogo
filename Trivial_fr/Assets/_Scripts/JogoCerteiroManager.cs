...

{
...

    {
        ...
        ResetColors(); // <-- TRANCA DE SEGURANÇA: Garante que estão visíveis antes de votar!
...

    {
        ...
        // Força a injeção de uma cor nova com 100% de opacidade (Alpha = 1f)

        for (int i = 0; i < answerBackgrounds.Length; i++)

        {

            if (answerBackgrounds[i] != null)

            {

                Color cor = answerBackgrounds[i].color;

                answerBackgrounds[i].color = new Color(cor.r, cor.g, cor.b, 1f);
...

        {
...

            {
                ...
                Color cor = answerTexts[i].color;

                answerTexts[i].color = new Color(cor.r, cor.g, cor.b, 1f);
